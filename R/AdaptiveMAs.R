...
modeDecomposition <- function(x, delta=.1, n=20, bandFraction=.1, ...) {
  beta <- cos(2*pi/n)
  ...
  trend <- xts(SMA(bandpass, 2*n), order.by=index(x))
  ...
  meanPeak <- bandFraction*SMA(peak, floor(2.5*n))
  meanValley <- bandFraction*SMA(valley, floor(2.5*n))
  meanPeakValley <- (meanPeak+meanValley)/2 
